...
const getNames = (arr) => {
  // Solution code here...
  let array = arr.map(val => val.name);
  let array1 = [];
  array.forEach(value => array1.push(value.split('').reverse().join('')));
  return array1;
};
...
const count = (target, input) => {
  ...
  let array = [];
  input.forEach(value => {
    array.push(value.filter(i => i === target))
  })
  let array1 = array.toString().split(',').join('').length;
  return array1;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let array = input.map((val) =>
    val.filter((n) => n % 5 === 0 && typeof n === 'number')
  );
  let array1 = array.map((val) =>
    val.map((data) => Math.pow(2, data))
  );
  return array1;
};
...
let findMaleAndFemale = (data) => {
  ...
  let array1 = array.map(item => item['name']).join(' and ');
  return array1;
};
